...
PARTITION BY customer\_id creates a separate ranking for each customer.

ORDER BY order\_date DESC places the latest order first within each customer group.

ROW\_NUMBER() = 1 returns the most recent order for every customer.
